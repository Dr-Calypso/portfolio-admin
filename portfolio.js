        ...
        if (gapi.auth2 && typeof gapi.auth2.getAuthInstance === 'function') {
          ...
          if (authInst) {
            ...
            try {
              ...
              const email = profile && profile.getEmail ? profile.getEmail() : null;
              const authResp = user && user.getAuthResponse ? user.getAuthResponse() : null;
              const idToken = authResp && (authResp.id_token || authResp.idToken);
              if(idToken){
                // Store raw ID token (short-lived). Consider refreshing periodically.
                try { localStorage.setItem('vcIdToken', idToken); } catch(_) {}
                window.__VC_ID_TOKEN = idToken;
              }
              if(email){
                if(email.toLowerCase() === 'doctor.calipso@gmail.com'){
                  localStorage.setItem('vcMode','1');
                  console.log('[vc-mode] Activated for privileged account', email);
                } else {
                  if(localStorage.getItem('vcMode') === '1'){
                    console.log('[vc-mode] Cleared (different user signed in):', email);
                  }
                  localStorage.removeItem('vcMode');
                  try { localStorage.removeItem('vcIdToken'); } catch(_) {}
                }
              }
            } catch(e){ console.warn('vcMode email detection failed', e); }
            const authResp = user && user.getAuthResponse ? user.getAuthResponse() : null;
